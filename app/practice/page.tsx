...
export default function PracticeProblems() {
    ...
    useEffect(() => {
        problemSets.forEach((item) => {
            setPageNumber((prev) => ({ ...prev, [item.title]: { page: item.page, total: 0 } }));
        })
    }, [])
                        ...
                        problemSets.map((item, index) => (
                            <div key={index} className="my-4 p-4 bg-[#222222] rounded-lg shadow-md">
                                <div className='flex flex-row justify-between'>
                                    <h2 className="text-[#f4c300] text-nowrap text-xl font-semibold mb-2">{item.title}</h2>
                                ...
                                <div className='flex flex-row justify-center flex-wrap'>
                                    <div className='flex flex-row align-middle justify-center mt-2 bg-white p-1 rounded-lg'>
                                        <Image 
                                            ...
                                            onClick={() => decrementPage(item.title)}
                                        />
                                        {pageNumber[item.title] && (
                                        ...
                                        <Image 
                                            ...
                                            onClick={() => incrementPage(item.title)}
                                        />
                                    </div>
                                </div>
                            </div>
